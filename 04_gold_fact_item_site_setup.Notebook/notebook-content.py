...
# --- is_excess: physical column for Excess Inventory Rate DAX measure ---
# Joins latest unrestricted stock from silver_mobo25 at item_no x site grain.
# is_excess = 1 where stock exceeds configured maximum_level (and maximum_level > 0).
# Items with maximum_level = 0 or NULL are excluded (unconfigured — not counted as excess).
# Pre-computed here to avoid NATURALINNERJOIN cross-table DAX in Direct Lake.
stock_latest = (
    spark.table("silver_mobo25")
    .groupBy("item_no", "site")
    .agg(F.max("Unrestricted").alias("latest_stock"))
)

fact_item_site_setup = (
    fact_item_site_setup
    .join(stock_latest, on=["item_no", "site"], how="left")
    .withColumn(
        "is_excess",
        F.when(
            (F.col("maximum_level") > 0) & (F.col("latest_stock") > F.col("maximum_level")),
            F.lit(1)
        ).otherwise(F.lit(0))
    )
    .drop("latest_stock")
